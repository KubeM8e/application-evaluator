...
func UnZipFile(sourceFile string) string {
	...
	if err != nil {
		log.Printf("Unzip - could not open reader: %v", err)
	}
	defer r.Close()
	...
	for _, f := range r.File {
		rc, errF := f.Open()
		if errF != nil {
			log.Printf("Unzip - could not open file: %v", err)
		}
		defer rc.Close()
		...
		if f.FileInfo().IsDir() {
			...
			if errOpen != nil {
				log.Printf("Unzip - could not open dst file: %v", err)
			}
			defer file.Close()

			_, err = io.Copy(file, rc)
			if err != nil {
				log.Printf("Unzip - could not copy files: %v", err)
			}
		}
	}
